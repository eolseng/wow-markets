# Derived from the Wails 2.13.0 installer template under the MIT License.
# The retained license is at companion/third_party/wails/LICENSE.
Unicode true
...
# Windows fixed versions must be numeric even when the product version is SemVer.
# The release workflow derives this value and passes it through the environment.
VIProductVersion "$%WOW_WINDOWS_FILE_VERSION%"
VIFileVersion    "$%WOW_WINDOWS_FILE_VERSION%"
...
Function LaunchUpdatedCompanionAsUser
    # Launch normally so the updated companion confirms the successful update
    # by reopening its window, matching Sparkle on macOS.
    ${StdUtils.ExecShellAsUser} $0 "$INSTDIR\${PRODUCT_EXECUTABLE}" "open" ""
FunctionEnd
